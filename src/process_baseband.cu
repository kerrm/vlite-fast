...
static uint64_t readtimes = 0;
static uint64_t writetimes = 0;
...
int main (int argc, char *argv[])
{
 ...
 if (key_out)
	multilog (log, LOG_INFO, "Will write to %d psrdada buffers.\n",
		NOUTBUFF);
 if (key_co)
	multilog (log, LOG_INFO, "Will write coadd to %d psrdada buffers.\n",
		1);
 ...
 while(true)
 {
	...
	if (major_skip) {
	 ...
	 // IF DO A LOCK_UNLOCK TRIP HERE
	 multilog( log, LOG_INFO, "Major skip detected! Lock-unlocking output buffers here!\n");
	}
